...
behaviour_info(callbacks) ->
    ...
    %%                      for compatibility {error, Reason} and {error, NewState}
    %%                       also supported
    ...
    [
     ...
     {login, 3}, %% State, Username, Password ->
                 %%{true OR false, State} | 'quit' (disconnect client)
     {check_user, 2}, %% State, Username ->
                      %% {ok, NewState} | {error, Reason, NewState},
                      %% 'quit' (disconnect client)
                      %% SECURE NOTE: Does not use check_user for testing is user
                      %% available, and return {ok, State} for all unexisting users,
                      %% because USER+PASS obfuscate is user exist and bad password,
                      %% or no user. This function should used for connection's
                      %% settings like USER requires SSL, USER can access from some
                      %% IP and etc
     ...
     {put_file, 4}, %% State, File Name, (append | write), Fun(Byte Count) ->
                    %% StateChange State, File Name, notification,
                    %% done (next command is arrived) | timeout (control_timeout is
                    %% passed, but connection works) | terminated (control connection
                    %% error) -> StateChange
     ...
     {disconnect, 2}, %% State, exit (QUIT command from client) or {error, Reason}  ->
                      %% *unused* State Change
     {set_alarm, 1}, %% Error -> ok
     {clean_alarm, 0}
    ];
behaviour_info(_) ->
    undefined.
